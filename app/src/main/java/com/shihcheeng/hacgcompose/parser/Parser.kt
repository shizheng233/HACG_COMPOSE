...
interface Parser {
    ...
    val service: HacgService

    suspend fun parserSinglePager(): Document

    suspend fun parserPagedPager(count: Int): Document

    suspend fun parser(count: Int): List<MainDataModel> {
        val innerDoc = if (count == 1) parserSinglePager() else parserPagedPager(count)
        val articles = innerDoc.body().select("#content").first()
        ...
        return articles
            ...
            .map { x ->
                ...
                val description = content?.getElementsByTag("p")?.first()?.removeClass("more-link")
                    ?.apply {
